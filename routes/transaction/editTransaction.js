...
app.patch('/transactions', auth, (req, res) => {
    let body = {}
    for (let key in req.body) {
        ...
        if (key == "id") {
            body[key] = Number(req.body[key])
            if (typeof body[key] == NaN || !body[key]) return res.status(400).send("Please input the right ID cuy")
        }
    }
